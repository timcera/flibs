...
struct lemon *lemp;
{
  static char templatename[] = "flempar.f90";
  char buf[1000];
  FILE *in;
  char *tpltname;
  char *cp;
...
int mhflag;                 /* True if generating makeheaders output */
{
  ...
  */
  ...
  if( lemp->errsym->useCnt ){
    fprintf(out,"    integer yy%d;\n",lemp->errsym->dtnum); lineno++;
  }
  free(stddt);
  free(types);
  fprintf(out,"end type YYMINORTYPE\n"); lineno++;
  *plineno = lineno;
}
...
int mhflag;     /* Output in makeheaders format if true */
{
  ...
  if( mhflag ){
    ...
    for(i=1; i<lemp->nterminal; i++){
      fprintf(out,"integer, parameter %s%-30s = %2d\n",prefix,lemp->symbols[i]->name,i);
      lineno++;
    }
    fprintf(out,"!#endif\n"); lineno++;
  }
  ...
  fprintf(out,"integer, parameter :: YYNOCODE = %d\n",lemp->nsymbol+1); lineno++;
  ...
  if( lemp->stacksize ){
    fprintf(out,"integer, parameter :: YYSTACKDEPTH = %d\n",lemp->stacksize); lineno++;
  }else{
    fprintf(out,"integer, parameter :: YYSTACKDEPTH = 100\n"); lineno++;
  }
  fprintf(out, "!#endif\n"); lineno++;
  if( mhflag ){
    fprintf(out,"!#if INTERFACE\n"); lineno++;
  ...
  fprintf(out,"integer, parameter :: YYNSTATE = %d\n",lemp->nstate); lineno++;
  fprintf(out,"integer, parameter :: YYNRULE = %d\n",lemp->nrule); lineno++;
  if( lemp->errsym->useCnt ){
    fprintf(out,"integer, parameter :: YYERRORSYMBOL = %d\n",lemp->errsym->index); lineno++;
    fprintf(out,"#define YYERRSYMDT yy%d\n",lemp->errsym->dtnum);  lineno++;
  } else {
    fprintf(out,"integer, parameter :: YYERRORSYMBOL = -1\n"); lineno++;
  }
  fprintf(out,"logical, parameter :: YYHASFALLBACK = %s\n",
  ...
  */
  ...
  fprintf(out,"integer, dimension(0:%d) :: yy_action = (/&\n", n-1); lineno++;
  for(i=j=0; i<n; i++){
  ...
  fprintf(out,"integer, dimension(0:%d) :: yy_lookahead = (/&\n", n-1); lineno++;
  for(i=j=0; i<n; i++){
  ...
  fprintf(out, "integer, parameter :: YY_SHIFT_USE_DFLT = %d\n", mnTknOfst-1); lineno++;
  n = lemp->nstate;
  ...
  fprintf(out, "integer, parameter :: YY_REDUCE_USE_DFLT = %d\n", mnNtOfst-1); lineno++;
  n = lemp->nstate;
  while( n>0 && lemp->sorted[n-1]->iNtOfst==NO_OFFSET ) n--;
  fprintf(out, "integer, parameter :: YY_REDUCE_MAX = %d\n", n-1); lineno++;
  fprintf(out, "integer, dimension(0:%d) :: yy_reduce_ofst = (/&\n", n-1); lineno++;
  for(i=j=0; i<n; i++){
    int ofst;
    stp = lemp->sorted[i];
    ofst = stp->iNtOfst;
  ...
  n = lemp->nstate;
  fprintf(out, "integer, dimension(0:%d) :: yy_default = (/&\n", n-1); lineno++;
  for(i=j=0; i<n; i++){
    stp = lemp->sorted[i];
    if( i<n-1) {
  ...
  */
  if( lemp->has_fallback ){
    fprintf(out, "integer, dimension(0:%d) :: yyFallback = (/&\n", lemp->nterminal); lineno++;
    for(i=0; i<lemp->nterminal; i++){
    ...
    fprintf(out, "integer, dimension(0) :: yyFallback\n");
  }
  tplt_xfer(lemp->name, in, out, &lineno);
  ...
  */
  ...
  fprintf(out,"    allocate( yyTokenName(0:%d) )\n",lemp->nsymbol-1);
  for(i=0; i<lemp->nsymbol; i++){
    fprintf(out,"    yyTokenName(%d) = \"%s\"\n", i, lemp->symbols[i]->name);
  }
  tplt_xfer(lemp->name,in,out,&lineno);
  ...
  */
  n=0;
  for(i=0, rp=lemp->rule; rp; rp=rp->next, i++){ n++; }

  fprintf(out,"    allocate( yyRuleName(0:%d) )\n",n-1);
  for(i=0, rp=lemp->rule; rp; rp=rp->next, i++){
    assert( rp->index==i );
    fprintf(out,"    yyRuleName(%d) = \"", i);
...
struct lemon *lemp;
{
  ...
  if( in ){
    for(i=1; i<lemp->nterminal && fgets(line,LINESIZE,in); i++){
      sprintf(pattern,"integer, parameter :: P_%s%-30s = %2d\n",prefix,lemp->symbols[i]->name,i);
  ...
  if( out ){
    for(i=1; i<lemp->nterminal; i++){
      fprintf(out,"integer, parameter :: P_%s%-30s = %2d\n",prefix,lemp->symbols[i]->name,i);
    }
    fclose(out);
  }
  return;
